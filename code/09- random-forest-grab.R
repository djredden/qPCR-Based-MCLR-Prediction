...
missranger_results <- miss_ranger_imputation |> 
  ...
  mutate(
    ...
    grab_mclr_binary = if_else(grab_mclr > grab_mclr_lod, "detected", "not_detected"),
    grab_mclr_binary = factor(grab_mclr_binary, levels = c("detected", "not_detected")),
  ...
  relocate(grab_mclr_binary, .after = grab_mclr) 
...
model_results <- setNames(lapply(mget(model_names), \(u) u[[2]]), model_names) |> 
  ...
  mutate(
    ...
    auc = map(predictions, ~ if (length(unique(.x$grab_mclr_binary)) == 2) {
      yardstick::roc_auc(data = .x, truth = grab_mclr_binary, .pred_detected)
    } else {
      NA
    }),
    roc = map(predictions, ~ if (length(unique(.x$grab_mclr_binary)) == 2) {
      yardstick::roc_curve(data = .x, truth = grab_mclr_binary, .pred_detected)
    } else {
      tibble::tibble(.threshold = NA, specificity = NA, sensitivity = NA)
    })
  )
...
write_csv(auc, "results/grab-auc.csv")
...
write_csv(roc_plot_data, "results/grab-roc-data.csv")
...
# write the data 
write_csv(var_imp_data_full, "results/var-imp-grab.csv")
